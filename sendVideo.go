...
func (t *BotT) SendVideoFromBytes(chatID int64, file []byte, caption string, keyboard KeyboardI) (*MessageT, error) {
	data := url.Values{}
	data.Add("chat_id", strconv.FormatInt(chatID, 10))
	data.Add("caption", caption)
	...
	return t.sendRawFile("sendVideo", data, "video", file)
}

func (t *BotT) SendVideoFromFile(chatID int64, file string, caption string, keyboard KeyboardI) (*MessageT, error) {
	f, err := os.Open(file)
	if err != nil {
		return nil, err
	}
	...
	return t.SendVideoFromBytes(chatID, bytes, caption, keyboard)
}
